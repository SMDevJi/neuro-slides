...
const SlideFrame = ({ slide, colors, setUpdatedSlider }: props) => {
    ...
    const handleAiSectionChange = async (userAiPrompt: string) => {
        ...
        try {
            const result = await axios.post('/api/projects/slides/ai-update', {
                userAiPrompt, oldHTML
            });
            const newHTML = result.data.newHTML.trim();
            ...
            console.error("AI generation failed:", err.response?.data?.message || err.message);
        }
    ...
    return (
        <div className='mb-5 relative'>
            <iframe
                ref={iframeRef}
                className="w-200 h-125 border-2 rounded-2xl "
